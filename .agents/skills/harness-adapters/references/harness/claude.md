...
When the project entry instead already carries a decline (`hasClaudeMdExternalIncludesApproved===false` with `hasClaudeMdExternalIncludesWarningShown===true`), the decline is left exactly as it is and only trust registers, so the worker launches with external imports disabled, the decision on record, and the registration prints a note naming the decline and its reset command.
