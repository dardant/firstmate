...
# A Herdr ship or scout closes its endpoint before its worktree is reaped and
# returned, because its pane's root shell lives inside that leased worktree;
# every other endpoint closes after the return.
...
# teardown_close_endpoint: close the task's endpoint, retire a Herdr
# presentation journal once its exact pane is confirmed gone, and stop before
# any record removal while a Herdr pane is not confirmed gone.
...
# A Herdr ship or scout pane is opened inside its leased worktree
# (bin/fm-spawn.sh spawn_treehouse_lease_worktree), so its root shell lives
# there, and both the reap below and `treehouse return --force` terminate
# every process inside the worktree, which would kill the pane out from under
# the focus-preserving close and the presentation journal's exact-pane
# confirmation. Its endpoint therefore closes here, after every landed-work
# refusal; every other endpoint keeps closing after the worktree is reaped and
# returned, which is what stops the agent first.
...
if [ "$TEARDOWN_ENDPOINT_CLOSED" = 0 ]; then
  teardown_close_endpoint
fi
if [ "$KIND" != secondmate ]; then
  if ! FM_HOME="$FM_HOME" FM_STATE_OVERRIDE="$STATE" FM_DATA_OVERRIDE="$DATA" \
