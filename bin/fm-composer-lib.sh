...
# composer either way, with one exception: a caller that knows the pane's
# harness names it in FM_COMPOSER_HARNESS, and a harness listed in
# FM_COMPOSER_FRAMED_HARNESSES always draws its composer inside a frame, so for
# it a bare agent glyph row with no `─` rule directly above and a closing rule
# below is not its composer.
