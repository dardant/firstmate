...
# (bin/fm-home-seed.sh) and for Herdr crewmate spawns, which must open their
# tab inside the worktree (bin/fm-spawn.sh spawn_treehouse_lease_worktree).
# Other crewmate spawns acquire their slot through the interactive
# pane-driven `treehouse get`, whose state entry is a live process lease
# (owner_pid plus owner_started_at, and `treehouse status` reports in-use
# from the processes actually running under the path).
...
# its own claim on top, on every backend: one file naming the task that took
# the slot, written by bin/fm-spawn.sh under the same project lock that
# allocates the slot and released by bin/fm-teardown.sh when the slot goes
# back to the pool. Moving
# the remaining crewmate spawns onto the durable lease is separate follow-up
# work.
#
