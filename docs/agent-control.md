...
- **Per-harness mechanics**: the key that cancels a running turn, how many times it must be delivered, whether the composer needs clearing afterwards, the command that exits the agent, how that harness's own exit confirmation dialog is recognized and answered, and which task kinds the adapter is verified to run.
...
When the harness answers its exit command with its own exit confirmation dialog - Claude's "Background work is running" when a background shell or task is running - `exit` answers it once with the adapter-verified choice that really exits and stops that background work, and leaves a dialog in any other shape untouched rather than guessing, so that exit ends `exit=unconfirmed`; `fm_control_exit_confirmation_key` in `bin/fm-control-lib.sh` owns the recognized shapes, and `relaunch` inherits the answer by stopping the old agent through `exit`.
An exit that delivers lifecycle input but cannot prove the agent stopped fails with `exit=unconfirmed` plus an `exit-dialog=` token saying whether a confirmation dialog was answered, reports the observed agent state and any interrupt cancellation claim, and never claims that nothing changed.
...
`bin/fm-send.sh` refuses every `--key` send and every typed text send that skips the inbox (slash commands and explicit backend targets) on such a pane, and on a pane it cannot capture, naming `exit` and `relaunch` as the recovery for a recorded task and saying an unrecorded pane must be cleared by hand.
The steering inbox's doorbell (`bin/fm-task-inbox-lib.sh`) skips such a pane with its own return code, so `bin/fm-send.sh` and the remote secondmate control plane report that the pane is parked on a launch dialog and name `exit` and `relaunch`, and an unacknowledged steer escalates instead of its Enter answering the dialog.
The doorbell also skips a pane it cannot capture, with the retry code, so the watcher re-rings later and escalates a pane that stays uncapturable while the steer stays durable in the inbox.
...
`bin/fm-busy-lib.sh`'s `fm_busy_launch_prompt_parked` owns the dialog signatures; the harness-agnostic check these guards share matches only the paired ones, so a worker's ordinary output quoting a single unpaired marker such as "Enter Gemini API Key" never refuses, and [runtime backend verification](verification/runtime-backends.md#claude-external-imports-dialog-answers) owns the evidence.
