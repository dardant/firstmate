...
A partition's peak memory is therefore the costliest shard's process rather than the sum of concurrent workers, which keeps it inside a 16 GB hosted runner; `bin/fm-lint.sh --help` owns why.
