...
`bin/fm-lint.sh` owns the canonical CI partitions, each running the same full source-aware ShellCheck analysis one ShellCheck process at a time, pinned versions, workflow validation, and backend-purity checks.
A partition's peak memory is therefore its costliest root rather than the sum of concurrent workers, which keeps it well inside a 16 GB hosted runner; `bin/fm-lint.sh --help` owns why.
