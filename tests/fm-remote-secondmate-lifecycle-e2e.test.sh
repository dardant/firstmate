...
cleanup() {
  ...
  # process group. Stop the published serving child's tree first, then every
  # remaining worker whose command line names this run's own code root - a
  # path unique to this fixture - until none is left, so no survivor keeps
  # writing remote-jobs while the fixture root is removed.
