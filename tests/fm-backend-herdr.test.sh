...
# unit-separated args, to $FAKE_HERDR_LOG) and returns the canned response for
# that call read from $FAKE_HERDR_RESPONSES/<n>.out, consumed IN ORDER (call 1
...
if [ "${1:-}" = status ] && [ "${2:-}" = --json ] && [ "${FAKE_HERDR_SCRIPT_STATUS:-0}" != 1 ]; then
  printf '{"client":{"version":"0.7.1","protocol":14},"server":{"running":true}}\n'
  exit 0
fi
if [ "${1:-}" = terminal ] && [ "${2:-}" = title ] && [ "${3:-}" = clear ]; then
  reason=${FAKE_HERDR_FOREGROUND_REASON:-no_foreground_client}
...
case "${1:-}" in
  status)
    if [ -e "$FAKE_HERDR_SERVER_MARKER" ]; then
  ...
  server)
    {
      for name in FM_HOME FM_ROOT_OVERRIDE FM_STATE_OVERRIDE FM_DATA_OVERRIDE FM_PROJECTS_OVERRIDE FM_CONFIG_OVERRIDE CURSOR_AGENT CURSOR_INVOKED_AS CLAUDECODE PI_CODING_AGENT FM_PI_HARNESS GROK_AGENT FM_SUPERVISION_MODEL FM_CREW_STATE_META_OVERRIDE FM_CREW_STATE_STATUS_OVERRIDE FAKE_HERDR_SENTINEL HERDR_SESSION; do
        eval 'value=${'"$name"'-<unset>}'
        printf '%s=%s\n' "$name" "$value"
      done
      printf 'args=%s\n' "$*"
    } > "$FAKE_HERDR_SERVER_ENV_LOG"
    : > "$FAKE_HERDR_SERVER_MARKER"
    ;;
esac
SH
  chmod +x "$fb/herdr"
...
# ($FAKE_HERDR_STATE) mutated with real jq. Modeled behaviors, all
...
# a pane with no registered agent). Every call is logged to $FAKE_HERDR_LOG in
...
# herdr_case <name> -> sets up FAKE_HERDR_LOG/FAKE_HERDR_RESPONSES/fb for one test,
...
test_version_check_accepts_current_protocol() {
  ...
  PATH="$fb:$PATH" FAKE_HERDR_LOG="$log" FAKE_HERDR_RESPONSES="$resp" FAKE_HERDR_SCRIPT_STATUS=1 \
...
test_version_check_refuses_old_protocol() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" FAKE_HERDR_LOG="$log" FAKE_HERDR_RESPONSES="$resp" FAKE_HERDR_SCRIPT_STATUS=1 \
...
test_cli_helper_sets_env_and_appends_trailing_session_flag() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  PATH="$fb:$PATH" FAKE_HERDR_LOG="$log" FAKE_HERDR_RESPONSES="$resp" \
...
test_recovery_grade_read_widens_only_at_its_own_boundary() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  husk=$(PATH="$fb:$PATH" FAKE_HERDR_LOG="$log" FAKE_HERDR_RESPONSES="$resp" FAKE_HERDR_SCRIPT_STATUS=1 \
...
#!/usr/bin/env bash
case "${FAKE_HERDR_STATUS_SHAPE:?}" in
...
SH
  ...
  for shape in legacy-equal legacy-older no-protocol stopped; do
    out=$(FAKE_HERDR_STATUS_SHAPE=$shape PATH="$dir/tools:/usr/bin:/bin" \
...
test_launcher_identity_absent_when_herdr_env_alone_is_set() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  ( PATH="$fb:$PATH" FAKE_HERDR_LOG="$log" FAKE_HERDR_RESPONSES="$resp" HERDR_ENV=1 \
    \
    bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_launcher_identity fmtest' "$ROOT" )
  status=$?
...
test_workspace_ensure_refuses_an_ambiguous_label_with_no_launcher() {
  ...
  printf '{"result":{"workspaces":[{"workspace_id":"w1","label":"firstmate"},{"workspace_id":"w7","label":"firstmate"}]}}\n' > "$resp/1.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" FAKE_HERDR_LOG="$log" FAKE_HERDR_RESPONSES="$resp" HERDR_SESSION=fmtest \
...
test_container_ensure_starts_server_and_workspace() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" FAKE_HERDR_LOG="$log" FAKE_HERDR_RESPONSES="$resp" FAKE_HERDR_SCRIPT_STATUS=1 HERDR_SESSION=fmtest \
...
test_server_ensure_scrubs_home_and_harness_identity() {
  ...
  PATH="$fb:$PATH" FAKE_HERDR_SERVER_ENV_LOG="$log" FAKE_HERDR_SERVER_MARKER="$marker" FAKE_HERDR_SENTINEL=kept \
    ...
    FM_CREW_STATE_META_OVERRIDE=/tmp/gone/lrt.meta FM_CREW_STATE_STATUS_OVERRIDE=/tmp/gone/lrt.status \
  ...
  assert_contains "$output" "FAKE_HERDR_SENTINEL=kept" "server_ensure removed an unrelated environment variable"
  ...
  pass "fm_backend_herdr_server_ensure: scrubs every Firstmate FM_* variable and harness identity without disturbing unrelated environment or session routing"
}
...
test_container_ensure_uses_secondmate_home_label() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" FM_HOME="$home" FAKE_HERDR_LOG="$log" FAKE_HERDR_RESPONSES="$resp" FAKE_HERDR_SCRIPT_STATUS=1 HERDR_SESSION=fmtest \
    bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_container_ensure /tmp' "$ROOT" )
...
test_projection_close_refuses_active_tab() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$(PATH="$fb:$PATH" FAKE_HERDR_LOG="$log" FAKE_HERDR_RESPONSES="$resp" \
    FAKE_HERDR_FOREGROUND_REASON=cleared \
    bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_projection_close_pane_focus_preserving fmtest w9:p2' "$ROOT" 2>&1)
  status=$?
...
# the pane-death close fixtures. The mover appends to $FAKE_MOVER_LOG and
# exits 9 unless $FAKE_MOVER_RESPONSE names a readable response file.
...
SH
  cat > "$dir/mover" <<'SH'
#!/usr/bin/env bash
printf '%s\t%s\t%s\n' "$1" "$2" "$3" >> "$FAKE_MOVER_LOG"
calls=$(wc -l < "$FAKE_MOVER_LOG" | tr -d ' ')
if [ "$calls" -ge 2 ] && [ -f "${FAKE_MOVER_RESPONSE_2:-}" ]; then
  cat "$FAKE_MOVER_RESPONSE_2"
  exit 0
fi
if [ -f "$FAKE_MOVER_RESPONSE" ]; then
  cat "$FAKE_MOVER_RESPONSE"
  exit 0
fi
exit 9
SH
...
test_projection_close_emptying_after_focus_uses_pane_death_without_move() {
  ...
  out=$(PATH="$fb:$PATH" FAKE_HERDR_LOG="$log" FAKE_HERDR_RESPONSES="$resp" \
    ...
    FAKE_MOVER_LOG="$dir/mover.log" FAKE_MOVER_RESPONSE="$dir/no-response" \
...
test_projection_close_emptying_before_focus_repositions_then_uses_pane_death() {
  ...
  out=$(PATH="$fb:$PATH" FAKE_HERDR_LOG="$log" FAKE_HERDR_RESPONSES="$resp" FAKE_HERDR_SCRIPT_STATUS=1 \
    FM_HERDR_PS_BIN="$dir/ps" FM_BACKEND_HERDR_WORKSPACE_MOVER="$dir/mover" \
    FAKE_MOVER_LOG="$dir/mover.log" FAKE_MOVER_RESPONSE="$dir/mover-response" \
    FM_BACKEND_HERDR_DEATH_CLOSE_POLLS=2 \
...
assert_projection_close_failed_removal_rolls_back_the_reposition() {
  ...
  out=$(PATH="$fb:$PATH" FAKE_HERDR_LOG="$log" FAKE_HERDR_RESPONSES="$resp" FAKE_HERDR_SCRIPT_STATUS=1 \
    FM_HERDR_PS_BIN="$dir/ps" FM_BACKEND_HERDR_WORKSPACE_MOVER="$dir/mover" \
    FAKE_MOVER_LOG="$dir/mover.log" FAKE_MOVER_RESPONSE="$dir/mover-response" \
    FAKE_MOVER_RESPONSE_2="$dir/mover-response-2" \
    FM_BACKEND_HERDR_DEATH_CLOSE_POLLS=2 \
    bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_projection_close_pane_focus_preserving fmtest w1:p1' "$ROOT" 2>&1)
  status=$?
  kill -KILL "$bgpid" 2>/dev/null || true; wait "$bgpid" 2>/dev/null || true
...
test_kill_emptying_non_focused_uses_pane_death() {
  ...
  out=$(PATH="$fb:$PATH" FAKE_HERDR_LOG="$log" FAKE_HERDR_RESPONSES="$resp" \
    FM_HERDR_PS_BIN="$dir/ps" FM_BACKEND_HERDR_WORKSPACE_MOVER="$dir/mover" \
    FAKE_MOVER_LOG="$dir/mover.log" FAKE_MOVER_RESPONSE="$dir/no-response" \
    FM_BACKEND_HERDR_DEATH_CLOSE_POLLS=2 FAKE_LOCK_LOG="$lock_log" \
    FAKE_LOCK_HELD="$lock_held" \
    bash -c '
      . "$0/bin/backends/herdr.sh"
      fm_backend_herdr_target_ready() { fm_backend_herdr_parse_target "$1"; }
      fm_backend_herdr_presentation_session_lock_path() { printf "%s" "$FAKE_LOCK_HELD.lock"; }
      fm_lock_try_acquire() {
        printf "acquire\n" >> "$FAKE_LOCK_LOG"
        : > "$FAKE_LOCK_HELD"
      }
      fm_lock_release() {
        [ -e "$FAKE_LOCK_HELD" ] || return 1
        rm -f "$FAKE_LOCK_HELD"
        printf "release\n" >> "$FAKE_LOCK_LOG"
      }
      eval "$(declare -f fm_backend_herdr_cli | sed "1s/fm_backend_herdr_cli/fm_backend_herdr_cli_locked/")"
      fm_backend_herdr_cli() {
        [ -e "$FAKE_LOCK_HELD" ] || return 97
...
test_endpoint_confirmed_gone_gates_on_structured_presence() {
  ...
  out=$(bash -c '
    . "$0/bin/backends/herdr.sh"
    fm_backend_herdr_cli() { printf "%s\n" "$FAKE_PRESENCE_RESPONSE"; return "${FAKE_PRESENCE_STATUS:-0}"; }
    check() {  # <label> <response> <status> <mode> <expected-rc>
      FAKE_PRESENCE_RESPONSE=$2 FAKE_PRESENCE_STATUS=$3
      rc=0
...
SH
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$(PATH="$fb:$PATH" FAKE_HERDR_LOG="$log" FAKE_HERDR_RESPONSES="$resp" FAKE_HERDR_SCRIPT_STATUS=1 \
    FM_BACKEND_HERDR_WORKSPACE_MOVER="$mover" FAKE_MOVER_LOG="$mover_log" \
...
test_projection_order_foreign_legacy_child_is_read_only() {
  ...
  cat > "$mover" <<'SH'
#!/usr/bin/env bash
echo called > "$FAKE_MOVER_CALLED"
exit 0
SH
  chmod +x "$mover"
  fb=$(make_herdr_fakebin "$dir")
  out=$(PATH="$fb:$PATH" FAKE_HERDR_LOG="$log" FAKE_HERDR_RESPONSES="$resp" \
    FM_BACKEND_HERDR_WORKSPACE_MOVER="$mover" FAKE_MOVER_CALLED="$dir/called" \
...
test_presentation_session_lock_path_is_shared_across_homes() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  path_a=$(PATH="$fb:$PATH" FAKE_HERDR_LOG="$log" FAKE_HERDR_RESPONSES="$resp" \
  ...
  path_b=$(PATH="$fb:$PATH" FAKE_HERDR_LOG="$log" FAKE_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_presentation_session_lock_path fmtest' "$ROOT") \
  ...
  path_other=$(PATH="$fb:$PATH" FAKE_HERDR_LOG="$log" FAKE_HERDR_RESPONSES="$resp" \
  ...
  if [ -L /tmp ] || [ "$(cd /tmp && pwd -P)" != /tmp ]; then
    ...
    path_tmp=$(PATH="$fb:$PATH" FAKE_HERDR_LOG="$log" FAKE_HERDR_RESPONSES="$resp" \
    ...
    path_private=$(PATH="$fb:$PATH" FAKE_HERDR_LOG="$log" FAKE_HERDR_RESPONSES="$resp" \
      bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_presentation_session_lock_path canon' "$ROOT") \
...
test_presentation_session_lock_path_rejects_malformed_socket() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  path=$(PATH="$fb:$PATH" FAKE_HERDR_LOG="$log" FAKE_HERDR_RESPONSES="$resp" \
...
test_workspace_find_matches_only_this_homes_own_label() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" FM_HOME="$home" FAKE_HERDR_LOG="$log" FAKE_HERDR_RESPONSES="$resp" \
...
test_wait_for_working_samples_budget_endpoint_without_final_sleep() {
  ...
  printf '{"result":{"agent":{"agent_status":"idle"}}}\n' > "$resp/6.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" FAKE_HERDR_LOG="$log" FAKE_HERDR_RESPONSES="$resp" FM_SLEEP_LOG="$sleep_log" \
...
test_send_text_submit_applies_herdr_minimum_confirm_budget() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" FAKE_HERDR_LOG="$log" FAKE_HERDR_RESPONSES="$resp" FM_SLEEP_LOG="$sleep_log" FM_BACKEND_HERDR_SUBMIT_POLLS=6 FM_BACKEND_HERDR_SUBMIT_MIN_SLEEP=0.6 \
...
test_rendered_busy_state_reads_the_cursor_busy_token() {
  ...
  printf '1\n' > "$resp/3.exit"
  fb=$(make_herdr_fakebin "$dir")
  idle_out=$( PATH="$fb:$PATH" FAKE_HERDR_LOG="$log" FAKE_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_rendered_busy_state default:w1:p2' "$ROOT" )
  busy_out=$( PATH="$fb:$PATH" FAKE_HERDR_LOG="$log" FAKE_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_rendered_busy_state default:w1:p2' "$ROOT" )
  fail_out=$( PATH="$fb:$PATH" FAKE_HERDR_LOG="$log" FAKE_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_rendered_busy_state default:w1:p2' "$ROOT" )
...
test_send_text_submit_slow_transition_within_one_enter_needs_no_extra_enter() {
  ...
  herdr_submit_identity_prefix "$resp" codex
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" FAKE_HERDR_LOG="$log" FAKE_HERDR_RESPONSES="$resp" FM_BACKEND_HERDR_SUBMIT_POLLS=3 \
...
SH
  ...
  out=$( PATH="$fb:$PATH" FM_ROOT_OVERRIDE="$neutral" FM_STATE_OVERRIDE="$state" \
    FAKE_HERDR_LOG="$log" FAKE_HERDR_RESPONSES="$resp" \
...
test_workspace_ensure_prunes_default_tab() {
  ...
  raw=$( PATH="$fb:$PATH" FAKE_HERDR_LOG="$log" FAKE_HERDR_STATE="$state" HERDR_SESSION=fmtest \
  ...
  ids=$( PATH="$fb:$PATH" FAKE_HERDR_LOG="$log" FAKE_HERDR_STATE="$state" HERDR_SESSION=fmtest \
...
EOF
    ...
    PATH="$fb:$PATH" FAKE_HERDR_LOG="$log" FAKE_HERDR_STATE="$state" HERDR_SESSION=fmtest \
      bash -c '
        . "$0/bin/backends/herdr.sh"
...
# makes - `session list --json` (echoes one session, name FAKE_SESSION_NAME,
# socket FAKE_SOCKET), `status --json`, and `agent get <pane>` (per-pane
# status read from $FAKE_AGENT_DIR/<key>.status, else agent_not_found).
...
case "$cmd $sub" in
  ...
  "session list")
    printf '{"sessions":[{"name":"%s","running":true,"default":false,"socket_path":"%s"}]}\n' \
      "${FAKE_SESSION_NAME:-default}" "${FAKE_SOCKET:-/tmp/fm-fake.sock}" ;;
  "agent get")
    if [ -n "${FAKE_READER_READY_FILE:-}" ] && [ ! -e "$FAKE_READER_READY_FILE" ]; then
      exit 9
    fi
    pane=${3:-}
    key=$(printf '%s' "$pane" | tr ':/.' '___')
    f="${FAKE_AGENT_DIR:-/tmp}/$key.status"
    if [ -f "$f" ]; then
      printf '{"result":{"agent":{"agent_status":"%s"}}}\n' "$(cat "$f")"
    else
      printf '{"error":{"code":"agent_not_found"}}\n' >&2
...
# the socket, streams the TAB-separated lines in $FAKE_READER_LINES to stdout
# (one projected event per line: pane_id\tworkspace_id\tagent_status\tagent),
# then exits $FAKE_READER_EXIT (default 0). A non-zero exit with no lines
...
if [ -n "${FAKE_READER_READY_FILE:-}" ]; then
  : > "$FAKE_READER_READY_FILE"
fi
printf '%s\n' "${FAKE_READER_ACK:-@subscribed}"
if [ -n "${FAKE_READER_LINES:-}" ] && [ -f "$FAKE_READER_LINES" ]; then
  cat "$FAKE_READER_LINES"
fi
exit "${FAKE_READER_EXIT:-0}"
SH
  chmod +x "$path"
  printf '%s\n' "$path"
}
...
test_wait_transition_reconcile_blocked_returns_record() {
  ...
  out=$(PATH="$fb:$PATH" TMPDIR="$temp" FM_BACKEND_HERDR_EVENTS_FORCE=1 FAKE_SESSION_NAME=sess FAKE_SOCKET="$dir/x.sock" FAKE_AGENT_DIR="$agent" \
    FM_BACKEND_HERDR_EVENT_READER="$reader" FAKE_READER_LINES="$lines" \
...
test_wait_transition_subscribes_before_reconcile() {
  ...
  rc=$(PATH="$fb:$PATH" FM_BACKEND_HERDR_EVENTS_FORCE=1 FAKE_SESSION_NAME=sess FAKE_SOCKET="$dir/x.sock" FAKE_AGENT_DIR="$agent" \
    FM_BACKEND_HERDR_EVENT_READER="$reader" FAKE_READER_LINES="$lines" FAKE_READER_READY_FILE="$ready" \
    bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_wait_transition sess 1 "$1" sess:wG:pQ; echo $?' "$ROOT" "$state" | tail -1)
...
test_wait_transition_stream_blocked_returns_record() {
  ...
  marker="$state/.herdr-escalated-sess_wG_pQ"
  out=$(PATH="$fb:$PATH" FM_BACKEND_HERDR_EVENTS_FORCE=1 FAKE_SESSION_NAME=sess FAKE_SOCKET="$dir/x.sock" FAKE_AGENT_DIR="$agent" \
    FM_BACKEND_HERDR_EVENT_READER="$reader" FAKE_READER_LINES="$lines" \
...
test_wait_transition_reader_failure_returns_2() {
  ...
  set_fake_agent "$agent" "wG:pQ" idle
  reader=$(make_fake_reader "$dir"); lines="$dir/lines"; : > "$lines"
  rc=$(PATH="$fb:$PATH" TMPDIR="$temp" FM_BACKEND_HERDR_EVENTS_FORCE=1 FAKE_SESSION_NAME=sess FAKE_SOCKET="$dir/x.sock" FAKE_AGENT_DIR="$agent" \
    FM_BACKEND_HERDR_EVENT_READER="$reader" FAKE_READER_LINES="$lines" FAKE_READER_EXIT=2 \
    bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_wait_transition sess 1 "$1" sess:wG:pQ; echo $?' "$ROOT" "$state" | tail -1)
...
test_wait_transition_bad_ack_returns_2_and_cleans_up() {
  ...
  set_fake_agent "$agent" "wG:pQ" idle
  reader=$(make_fake_reader "$dir"); lines="$dir/lines"; : > "$lines"
  result=$(PATH="$fb:$PATH" TMPDIR="$temp" FM_BACKEND_HERDR_EVENTS_FORCE=1 FAKE_SESSION_NAME=sess FAKE_SOCKET="$dir/x.sock" FAKE_AGENT_DIR="$agent" \
    FM_BACKEND_HERDR_EVENT_READER="$reader" FAKE_READER_LINES="$lines" FAKE_READER_ACK=invalid \
